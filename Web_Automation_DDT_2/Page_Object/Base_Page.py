...
class BasePage:
    ...
    logger = Logger(config_dir.LOG_PATH + '/web_{0}.log'.format
    (time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime())), 'DEBUG', 'DEBUG')
    ...
    def switch_window(self, wait, requence, name=None):
        if not name:
            handles = self.driver.window_handles
            current_handle = self.driver.current_window_handle
            WebDriverWait(self.driver, wait, requence).until(EC.new_window_is_opened(current_handle))
