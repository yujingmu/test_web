...
class BasePage:
    ...
    def _screen_shot(self, model="model"):                         # 函数名添加下划线表示该函数只在内部使用
        ...
        file_path = config_dir.IMG_PATH + "/{0}.png"\
            .format(time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime()))
        self.driver.save_screenshot(file_path)
        logging.info("{0}错误截图成功，路径为: {1}".format(model, file_path))

    def wait_element_click(self, locator, model="model", wait=30, requence=0.2):
        try:
    ...
    def wait_element_presence(self, locator, model="model", wait=30, requence=0.5):
        ...
        # 日志
            logging.error("等待元素存在异常{0}".format(locator))
        # 截图   存放地址，图片名字
            self._screen_shot(model)
            raise

    def wait_element_quick_presence(self, locator, model="model", wait=5, requence=0.2):
        try:
    ...
    def wait_iframe_available_and_switch_to_it(self, locator, model="model", wait=30, requence=0.5):
        try:
            element = WebDriverWait(self.driver, wait, requence)
